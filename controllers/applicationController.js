...
export const employerGetAllApplications = catchAsyncError(
  async (req, res, next) => {
    ...
    const { _id } = req.user;
    const applications = await Application({ "employerId.user": _id });
    res.status(200).json({
      success: true,
      applications,
    });
...
export const jobseekerGetAllApplications = catchAsyncError(
  async (req, res, next) => {
    ...
    const { _id } = req.user;
    const applications = await Application({ "applicantID.user": _id });
    res.status(200).json({
      success: true,
      applications,
    });
...
export const postApplication = catchAsyncError(async (req, res, next) => {
  ...
  if (!allowedFormats.includes(resume.mimetype)) {
    return next(
      new ErrorHandler(
        "Invalid File Type, Please upload png, jg or webp format"
      )
    );
  }
